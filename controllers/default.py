...
def rank1():
    x=request.args(0)
    y=int(x)
    db.auth_user[auth.user.id] = dict(marks1=y)
    z=y+db.auth_user[auth.user.id].marks2
    db.auth_user[auth.user.id]= dict(marks=z)
    rows=db().select(db.auth_user.ALL,orderby=~db.auth_user.marks1)
    return locals()

def rank2():
    x=request.args(0)
    y=int(x)
    z=y+db.auth_user[auth.user.id].marks1
    db.auth_user[auth.user.id] = dict(marks2=y)
    db.auth_user[auth.user.id]= dict(marks=z)
    rows=db().select(db.auth_user.ALL,orderby=~db.auth_user.marks2)
    return locals()

def rank():
    rows=db().select(db.auth_user.ALL,orderby=~db.auth_user.marks)
    return locals()
...
def kirti():
    form=SQLFORM(db.ques).process()
    rows=db(db.ques).select()
    return locals()
